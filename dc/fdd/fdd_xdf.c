...
BRESULT fddxdf_setfdi(FDDFILE fdd, const OEMCHAR *fname, int ro) {
	...
	fdsize = 0;
	r = 0;
	fh = ccfile_open_rb(fname);
	if (fh != CCFILEH_INVALID) {
		fdsize = ccfile_getsize(fh);
		r = ccfile_read(fh, &fdi, sizeof(fdi));
		ccfile_close(fh);
	}
	if (r != sizeof(fdi)) {
		return(FAILURE);
...
BRESULT fddxdf_read(FDDFILE fdd) {
	...
	hdl = ccfile_open_rb(fdd->fname);
	if (hdl == CCFILEH_INVALID) {
		fddlasterror = 0xe0;
		return(FAILURE);
	}
	if ((ccfile_seek(hdl, seekp, FSEEK_SET) != seekp) ||
		(ccfile_read(hdl, fdc.buf, secsize) != secsize)) {
		ccfile_close(hdl);
		fddlasterror = 0xe0;
		return(FAILURE);
	}
...
BRESULT fddxdf_write(FDDFILE fdd) {
	...
	hdl = ccfile_open(fdd->fname);
	if (hdl == CCFILEH_INVALID) {
		fddlasterror = 0xc0;
		return(FAILURE);
	}
	if ((ccfile_seek(hdl, seekp, FSEEK_SET) != seekp) ||
		(ccfile_write(hdl, fdc.buf, secsize) != secsize)) {
		ccfile_close(hdl);
		fddlasterror = 0xc0;
		return(FAILURE);
	}
