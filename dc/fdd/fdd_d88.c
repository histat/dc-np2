...
static BRESULT d88trk_flushdata(D88TRK trk) {
	...
	if ((ccfile_seek(fh, trk->fptr, FSEEK_SET) != trk->fptr) ||
		(ccfile_write(fh, trk->buf, trk->size) != trk->size)) {
		goto dtfd_err2;
	}
	ccfile_close(fh);
	trk->write = FALSE;
...
static BRESULT d88trk_read(D88TRK trk, FDDFILE fdd, UINT track, UINT type) {
	...
	if ((ccfile_seek(fh, (long)fptr, FSEEK_SET) != (long)fptr) ||
		(ccfile_read(fh, trk->buf, size) != size)) {
		goto dtrd_err2;
	}
	ccfile_close(fh);
...
BRESULT fddd88_set(FDDFILE fdd, const OEMCHAR *fname, int ro) {
	...
	rsize = ccfile_read(fh, &fdd->inf.d88.head, sizeof(fdd->inf.d88.head));
	ccfile_close(fh);
...
static int fileappend(CCFILEH hdl, FDDFILE fdd,
	...
	while(length) {
		...
		length -= size;
		ccfile_seek(hdl, ptr + length, 0);
		rsize = ccfile_read(hdl, tmp, size);
		ccfile_seek(hdl, ptr + length + apsize, 0);
		ccfile_write(hdl, tmp, rsize);
	}
...
static void endoftrack(UINT fmtsize, UINT8 sectors) {
	...
	hdl = ccfile_open(fddfile[fdc.us].fname);
	if (hdl == CCFILEH_INVALID) {
		return;
	}
	lastpointer = ccfile_seek(hdl, 0, FSEEK_END);
	...
	ccfile_seek(hdl, fpointer, 0);
	ccfile_write(hdl, d88trk.buf, fmtsize);
	ccfile_seek(hdl, 0, FSEEK_SET);
	ccfile_write(hdl, &fdd->inf.d88.head, sizeof(fdd->inf.d88.head));
	ccfile_close(hdl);
