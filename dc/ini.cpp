...
static BOOL inireadcb(void *arg, const OEMCHAR *para, const OEMCHAR *key, const OEMCHAR *data)
{
	const PFTBL	*p;
...
void ini_read(const OEMCHAR *path, const OEMCHAR *title, const PFTBL *tbl, UINT count)
{
	_PFARG	iniarg;
...
void ini_write(const OEMCHAR *path, const OEMCHAR *title, const PFTBL *tbl, UINT count)
{
	TEXTFILEH		tfh;
	const PFTBL		*p;
	const PFTBL		*pterm;
	...
	while (p < pterm) {
		...
		case PFTYPE_BOOL:
			milstr_ncpy(work, (*((UINT8 *)p->value))?str_true:str_false, NELEMENTS(work));
			break;
...
const OEMCHAR ini_title[] = OEMTEXT("NekoProjectII");
const OEMCHAR inifile[] = OEMTEXT("np2.cfg");
...
void initload(void)
{
	...
	if (files == 1 && load_from_vmu(res, fname, buf, &size)) {
		...
		ini_read(fname, ini_title, iniitem, NELEMENTS(iniitem));
...
void initsave(void)
{
	...
	ini_write(fname, ini_title, iniitem, NELEMENTS(iniitem));
